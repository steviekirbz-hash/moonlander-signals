...
const SCORE_CONFIG: { [key: string]: { label: string; bg: string; glow: string; textColor: string; icon: React.ElementType } } = {
...
const FearGreedCard = ({ data }: { data: FearGreedData | null | undefined }) => {
  if (!data) return null;
...
// ADX Cell Component
const ADXCell = ({ adx }: { adx: Asset['adx'] }) => {
...
// DeMark Cell Component
const DemarkCell = ({ demark }: { demark: Asset['demark'] }) => {
  if (!demark || !demark.count || demark.count === 0) {
    return <span className="text-slate-600">—</span>;
  }
  
  const isComplete = demark.completed;
  const isBullish = demark.signal === 'bullish';
  const isBearish = demark.signal === 'bearish';
  
  let colorClass = 'text-slate-400';
  if (demark.count >= 7) {
    colorClass = isBullish ? 'text-emerald-400' : isBearish ? 'text-red-400' : 'text-yellow-400';
  }
  ...
  return (
    <div className="flex items-center gap-1">
      ...
      {isComplete && (
        <Flame size={14} className={isBullish ? 'text-emerald-400' : 'text-red-400'} />
      )}
    </div>
  );
};

// Volume Cell Component
const VolumeCell = ({ volume }: { volume: Asset['relative_volume'] }) => {
...
const ScoreBadge = ({ score }: { score: number }) => {
  ...
  return (
    <div className={`${config.bg} ${config.glow} px-2 py-1 rounded-lg flex items-center gap-1.5 font-semibold text-[10px] tracking-wide transition-all duration-300 hover:scale-105 ${config.textColor}`}>
      <Icon size={12} />
...
const FireIndicator = ({ aligned, max = 4 }: { aligned: number; max?: number }) => {
  const flames = [];
  for (let i = 0; i < max; i++) {
    flames.push(
      <Flame
...
const RSICell = ({ rsi }: { rsi: Asset['rsi'] }) => {
...
const ExpandedRow = ({ asset }: { asset: Asset }) => {
  return (
    <div className="bg-slate-900/50 border-t border-slate-700/50 px-4 py-4 animate-slideDown">
      <div className="grid grid-cols-1 md:grid-cols-4 gap-3">
                  ...
                  <div className="w-16 h-1.5 bg-slate-700 rounded-full overflow-hidden">
                    <div 
                      className={`h-full rounded-full ${
                        value < 30 ? 'bg-emerald-500' : value > 70 ? 'bg-red-500' : 'bg-slate-500'
                      }`}
                      style={{ width: `${value}%` }}
                    />
                  </div>
                  <span className={`text-xs font-mono font-bold w-6 text-right ${
                    value < 30 ? 'text-emerald-400' : value > 70 ? 'text-red-400' : 'text-slate-400'
        ...
        <div className="bg-slate-800/50 rounded-xl p-3 border border-slate-700/30">
          <h4 className="text-slate-400 text-xs uppercase tracking-wider mb-2 flex items-center gap-2">
            <BarChart3 size={12} /> ADX Trend Strength
          </h4>
          {asset.adx ? (
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                <span className="text-slate-400 text-xs">ADX Value</span>
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs font-semibold ${
                  asset.adx.trend_strength === 'strong' || asset.adx.trend_strength === 'very strong' 
              ...
              <div className="flex items-center justify-between">
                ...
                <span className={`text-xs font-semibold ${
                  asset.adx.direction === 'bullish' ? 'text-emerald-400' : 
                  asset.adx.direction === 'bearish' ? 'text-red-400' : 'text-slate-400'
                }`}>{asset.adx.direction}</span>
              </div>
            </div>
        ...
        <div className="bg-slate-800/50 rounded-xl p-3 border border-slate-700/30">
          <h4 className="text-slate-400 text-xs uppercase tracking-wider mb-2 flex items-center gap-2">
            <AlertTriangle size={12} /> DeMark Sequential
          </h4>
            ...
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between">
                ...
                <span className="text-slate-300 text-xs">
                  {asset.demark.type === 'S' ? 'Sell Setup (Bullish)' : 'Buy Setup (Bearish)'}
                </span>
              </div>
              {asset.demark.completed && (
                <div className="flex items-center gap-2 mt-2 p-2 bg-slate-900/50 rounded-lg">
                  ...
                  <span className={`text-xs font-semibold ${
                    asset.demark.signal === 'bullish' ? 'text-emerald-400' : 'text-red-400'
                  }`}>
                    {asset.demark.signal === 'bullish' ? 'Bullish Reversal Signal!' : 'Bearish Reversal Signal!'}
                  </span>
                </div>
              )}
            </div>
        ...
        <div className="bg-slate-800/50 rounded-xl p-3 border border-slate-700/30">
          <h4 className="text-slate-400 text-xs uppercase tracking-wider mb-2 flex items-center gap-2">
            <Volume2 size={12} /> Relative Volume
          </h4>
            ...
            <div className="space-y-2">
              <div className="flex items-center justify-between">
                ...
                <span className={`font-bold text-lg ${
                  asset.relative_volume.signal === 'high' ? 'text-emerald-400' :
                  asset.relative_volume.signal === 'low' ? 'text-red-400' : 'text-slate-300'
                }`}>{asset.relative_volume.display}</span>
              </div>
              <div className="flex items-center justify-between">
                <span className="text-slate-400 text-xs">Signal</span>
                <span className={`text-xs font-semibold ${
                  asset.relative_volume.signal === 'high' ? 'text-emerald-400' :
                  asset.relative_volume.signal === 'low' ? 'text-red-400' : 'text-slate-400'
                }`}>
                  {asset.relative_volume.signal === 'high' ? 'Above Average' :
                   asset.relative_volume.signal === 'low' ? 'Below Average' : 'Normal'}
                </span>
              </div>
            </div>
          ) : <span className="text-slate-600">No data</span>}
...
const AssetRow = ({ asset, isExpanded, onToggle }: { asset: Asset; isExpanded: boolean; onToggle: () => void }) => {
  return (
    <>
      <tr 
      ...
      >
        <td className="px-3 py-2.5">
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 rounded-lg bg-gradient-to-br from-slate-700 to-slate-800 flex items-center justify-center font-bold text-[10px] border border-slate-600/50 group-hover:border-cyan-500/30 transition-colors">
              {asset.symbol.slice(0, 3)}
            </div>
            <div>
              <div className="font-semibold text-white text-xs group-hover:text-cyan-400 transition-colors">{asset.symbol}</div>
        ...
        <td className="px-3 py-2.5 hidden lg:table-cell">
          <ADXCell adx={asset.adx} />
        </td>
        <td className="px-3 py-2.5 hidden lg:table-cell">
          <DemarkCell demark={asset.demark} />
        </td>
        <td className="px-3 py-2.5 hidden md:table-cell">
          <VolumeCell volume={asset.relative_volume} />
        </td>
        <td className="px-3 py-2.5">
          {isExpanded ? <ChevronUp size={16} className="text-cyan-400" /> : <ChevronDown size={16} className="text-slate-600 group-hover:text-cyan-400 transition-colors" />}
        </td>
      </tr>
      {isExpanded && (
        <tr>
...
const MarketSentiment = ({ assets, fearGreed }: { assets: Asset[]; fearGreed: FearGreedData | null | undefined }) => {
    ...
    <div className="bg-gradient-to-br from-slate-900 via-slate-800 to-slate-900 rounded-2xl p-5 border border-slate-700/50">
      <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
        ...
        <div className="flex gap-6 text-center">
          <div>
            <div className="text-emerald-400 text-xl font-bold">{bullish}</div>
            <div className="text-[9px] text-slate-500 uppercase">Bullish</div>
          </div>
          <div>
            <div className="text-slate-400 text-xl font-bold">{neutral}</div>
            <div className="text-[9px] text-slate-500 uppercase">Neutral</div>
          </div>
          <div>
            <div className="text-red-400 text-xl font-bold">{bearish}</div>
...
const StatsCard = ({ icon: Icon, label, value, subValue, color }: { icon: React.ElementType; label: string; value: number | string; subValue?: string; color: string }) => (
  <div className="bg-gradient-to-br from-slate-900 to-slate-800 rounded-xl p-4 border border-slate-700/50 hover:border-cyan-500/30 transition-all duration-300 hover:scale-[1.02] group">
    <div className="flex items-start justify-between mb-2">
      <div className={`p-2 rounded-lg bg-slate-800 ${color} group-hover:scale-110 transition-transform`}>
        <Icon size={16} />
      </div>
    </div>
    ...
    <div className="min-h-screen bg-slate-950 text-white">
      ...
      <div className="relative z-10">
          ...
          <div className="max-w-7xl mx-auto px-4 py-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h1 className="text-lg font-black tracking-tight bg-gradient-to-r from-white via-cyan-200 to-white bg-clip-text text-transparent">
                    MOONLANDER SIGNALS
                  </h1>
              ...
              <div className="flex items-center gap-3">
                <div className="hidden sm:flex items-center gap-2 text-slate-500 text-xs">
                  <Clock size={12} />
                  <span>Updated {data?.generated_at ? new Date(data.generated_at).toLocaleTimeString() : '--:--'}</span>
                </div>
                <button 
                  onClick={loadData}
                  className="p-2 rounded-lg bg-slate-800 hover:bg-slate-700 border border-slate-700 hover:border-cyan-500/50 transition-all group"
                >
                  <RefreshCw size={16} className={`text-slate-400 group-hover:text-cyan-400 transition-colors ${isLoading ? 'animate-spin' : ''}`} />
                </button>
              </div>
            </div>
          </div>
        ...
        <main className="max-w-7xl mx-auto px-4 py-5">
          {/* Stats Grid - Now includes Fear & Greed */}
          <div className="grid grid-cols-2 md:grid-cols-5 gap-3 mb-5">
            ...
            <StatsCard 
              icon={TrendingUp} 
              label="Bullish" 
              value={assets.filter(a => a.score > 0).length}
              subValue="Long bias"
          ...
          {/* Market Sentiment */}
          <div className="mb-5">
            <MarketSentiment assets={assets} fearGreed={data?.fear_greed} />
          </div>
          ...
          <div className="flex flex-col md:flex-row gap-3 mb-4">
            <div className="relative flex-1 max-w-xs">
              ...
              <input
                ...
                className="w-full pl-9 pr-3 py-2 bg-slate-800/50 border border-slate-700 rounded-lg text-xs text-white placeholder-slate-500 focus:outline-none focus:border-cyan-500/50"
              />
            </div>
          ...
          {/* Main Table */}
          <div className="bg-gradient-to-br from-slate-900/80 to-slate-900/40 rounded-2xl border border-slate-800/50 overflow-hidden backdrop-blur-sm">
            <div className="overflow-x-auto">
              <table className="w-full">
                <thead>
                  <tr className="border-b border-slate-700/50 bg-slate-900/50">
                    <th className="px-3 py-2.5 text-left text-[9px] font-semibold text-slate-400 uppercase tracking-wider cursor-pointer hover:text-cyan-400" onClick={() => handleSort('symbol')}>
                      <div className="flex items-center gap-1">Asset <ArrowUpDown size={10} /></div>
                    </th>
                    <th className="px-3 py-2.5 text-left text-[9px] font-semibold text-slate-400 uppercase tracking-wider cursor-pointer hover:text-cyan-400" onClick={() => handleSort('price')}>
                      <div className="flex items-center gap-1">Price <ArrowUpDown size={10} /></div>
                    </th>
                    <th className="px-3 py-2.5 text-left text-[9px] font-semibold text-slate-400 uppercase tracking-wider cursor-pointer hover:text-cyan-400" onClick={() => handleSort('score')}>
                      <div className="flex items-center gap-1">Signal <ArrowUpDown size={10} /></div>
                    </th>
                    ...
                    <th className="px-3 py-2.5 text-left text-[9px] font-semibold text-slate-400 uppercase tracking-wider hidden lg:table-cell cursor-pointer hover:text-cyan-400" onClick={() => handleSort('adx')}>
                      <div className="flex items-center gap-1">ADX <ArrowUpDown size={10} /></div>
                    </th>
                ...
                <tbody>
                  {filteredAssets.map((asset) => (
                    <AssetRow 
                      ...
                      onToggle={() => setExpandedRow(expandedRow === asset.symbol ? null : asset.symbol)}
                    />
                  ))}
                </tbody>
              </table>
          ...
          <div className="mt-5 p-4 bg-slate-900/50 rounded-xl border border-slate-800/50">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 text-[10px]">
              ...
              <div className="flex items-center gap-2">
                <span className="text-orange-400 font-mono">ADX &gt;40</span>
                <span className="text-slate-400">Strong Trend</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-yellow-400 font-mono">S:9! / B:9!</span>
                <span className="text-slate-400">DeMark Exhaustion</span>
              </div>
            </div>
          </div>
